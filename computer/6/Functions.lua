...
function TF.SaveKnowledge(Knowledge)
    ...
    KFile = fs.open(Knowledge["Result"][1]["Itemname"],"w")
    KFile.write(Knowledge["Result"])


end

function TF.SearchKnowledge(ItemName)
    local ReturnList = {}
    KnowledgeList = fs.list("CraftingKnowledge")
    if #KnowledgeList == 0 then
        return nil -- you dont know anything, stop
    end
    for i = 0, #KnowledgeList do 
        if KnowledgeList[i] == ItemName then
            ReturnList[#ReturnList + 1] = KnowledgeList[i]
        end
    end
